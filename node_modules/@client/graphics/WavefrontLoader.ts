...
const ops: {[name: string]: WavefrontOp} = {
	...
	f: (loader, line, op, args) => {
		...
		for (let point of points) {
			...
			let simple = parseInt(point);
			if (!isNaN(simple)) {
				v = vt = vn = simple;
			}

			let dsplit = point.split("/").map(parseInt);
			...
			console.log([].concat(vertex, texcoord, normal));

			loader.buffer.append([].concat(vertex, texcoord, normal));
		}
	}
};
