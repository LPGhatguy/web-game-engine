...
engine.onLoad.subscribe(() => {
	...
	lightShader.addUniforms("u_light", "u_lightColor");
	...
	prepassShader.use();
	gl.uniformMatrix4fv(engine.currentProgram.uniforms["transformProjection"], false, cam.getProjectionMatrix().array);
});
...
engine.onDraw.subscribe((dt: number) => {
	...
	for (let light of lights) {
		let [x, y, z, w, r, g, b] = light;
		gl.uniform4f(lightShader.uniforms["u_light"], x, y, z, w);
		gl.uniform3f(lightShader.uniforms["u_lightColor"], r, g, b);
		fsquad.draw();
	}
});
